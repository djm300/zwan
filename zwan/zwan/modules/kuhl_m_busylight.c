...
const BUSYLIGHT_COMMAND_STEP kuhl_m_busylight_steps_MangoHack[] = {
...
NTSTATUS kuhl_m_busylight_init()
{
	...
	BOOL isKbFR = (PtrToUlong(GetKeyboardLayout(0)) >> 16) == 0x40c, isMango = FALSE;
	if(isBusyLight = kull_m_busylight_devices_get(&kuhl_m_busylight_devices, NULL, BUSYLIGHT_CAP_LIGHT, TRUE))
	{
		...
		{
			isMango = ((!(cur->id % 2) && isKbFR) || ((cur->id % 2) && !isKbFR));
			kull_m_busylight_request_send(cur, isMango ? kuhl_m_busylight_steps_MangoHack : kuhl_m_busylight_steps_ReneCotyHack, isMango ? ARRAYSIZE(kuhl_m_busylight_steps_MangoHack) : ARRAYSIZE(kuhl_m_busylight_steps_ReneCotyHack), FALSE);
		}
	}
