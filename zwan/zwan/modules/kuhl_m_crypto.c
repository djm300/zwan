...
NTSTATUS kuhl_m_crypto_init()
{
	...
	if(kuhl_m_crypto_hRsaEnh = LoadLibrary(L"rsaenh"))
	{
		...
		{
			if((zwan_NT_MAJOR_VERSION > 5) && !kuhl_m_crypto_hNCrypt)
			{
...
NTSTATUS kuhl_m_crypto_c_sc_auth(int argc, wchar_t * argv[])
{
	...
	CERT_RDN_ATTR rgNameAttr[] = {
		...
		{szOID_ORGANIZATION_NAME, CERT_RDN_UNICODE_STRING, {sizeof(zwan) - sizeof(wchar_t), (PBYTE) zwan}},
	...
	if(keyInfos.pwszContainerName = kull_m_string_getRandomGUID())
	{
		...
		else PRINT_ERROR(L"/caname:CA-MANGO needed\n");
...
NTSTATUS kuhl_m_crypto_c_cert_to_hw(int argc, wchar_t * argv[])
{
				...
				if(isExported)
				{
					...
					if(keyInfos.pwszContainerName = kull_m_string_getRandomGUID())
					{
						...
						if(kull_m_string_args_byName(argc, argv, L"pin", &szPin, NULL))
						{
							...
							{
								if(CryptAcquireContext(&hProvCERT, NULL, keyInfos.pwszProvName, keyInfos.dwProvType, ((zwan_NT_MAJOR_VERSION > 5) ? CRYPT_DEFAULT_CONTAINER_OPTIONAL : 0)))
								{
									if(CryptSetProvParam(hProvCERT, PP_KEYEXCHANGE_PIN, (const BYTE *) aPin, 0))
	...
	else PRINT_ERROR(L"/name:mango needed\n");
...
NTSTATUS kuhl_m_crypto_p_capi(int argc, wchar_t * argv[])
{
	...
	currentReference4001 = kull_m_patch_getGenericFromBuild(Capi4001References, ARRAYSIZE(Capi4001References), zwan_NT_BUILD_NUMBER);
	currentReference4000 = kull_m_patch_getGenericFromBuild(Capi4000References, ARRAYSIZE(Capi4000References), zwan_NT_BUILD_NUMBER);
...
NTSTATUS kuhl_m_crypto_p_cng(int argc, wchar_t * argv[])
{
	...
	if(kuhl_m_crypto_hNCrypt)
	{
		...
		{
			K_NCryptFreeObject(hProvider);
			kull_m_patch_genericProcessOrServiceFromBuild(CngReferences, ARRAYSIZE(CngReferences), L"KeyIso", (zwan_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_8) ? L"ncrypt.dll" : L"ncryptprov.dll", TRUE);
		}
	}
	else PRINT_ERROR(L"No CNG\n");
	return STATUS_SUCCESS;
