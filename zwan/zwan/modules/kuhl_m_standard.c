...
const KUHL_M_C kuhl_m_c_standard[] = {
	...
	{kuhl_m_standard_exit,		L"exit",		L"Quit zwan"},
	...
	{kuhl_m_standard_log,		L"log",			L"Log zwan input/output to file"},
...
{
	PCWCHAR filename = (kull_m_string_args_byName(argc, argv, L"stop", NULL, NULL) ? NULL : (argc ? argv[0] : zwan_DEFAULT_LOG));
...
NTSTATUS kuhl_m_standard_version(int argc, wchar_t * argv[])
{
	...
	{
		kprintf(
			L"\n" zwan L" " zwan_VERSION L" (arch " zwan_ARCH L")\n"
			L"Windows NT %u.%u build %u (arch x%s)\n"
			L"msvc %u %u\n",
			zwan_NT_MAJOR_VERSION, zwan_NT_MINOR_VERSION, zwan_NT_BUILD_NUMBER, isWow64 ? L"64" : L"86", _MSC_FULL_VER, _MSC_BUILD
			);
	}
	return STATUS_SUCCESS;
}
