							...
							if(!RtlEqualUnicodeString(&usXCACHECONF, &ticket->TargetDomainName, TRUE))
							{
								...
								if(isSave || isInject)
								{
									if(BerApp_KrbCred = kuhl_m_kerberos_ticket_createAppKrbCred(ticket, TRUE))
									{
										...
										{
											if(saveFilename = kuhl_m_kerberos_ccache_generateFileName(i, ticket, zwan_KERBEROS_EXT))
											{
...
wchar_t * kuhl_m_kerberos_ccache_generateFileName(const DWORD index, PMANGO_KERBEROS_TICKET ticket, LPCWSTR ext)
{
	wchar_t * buffer;
