...
VOID WDBGAPI WinDbgExtensionDllInit(PWINDBG_EXTENSION_APIS lpExtensionApis, USHORT usMajorVersion, USHORT usMinorVersion)
{
	...
	dprintf("\n"
		"  .#####.   " zwan_FULL_A "\n"
		" .## ^ ##.  " zwan_SECOND_A " - Windows build %hu\n"
		" ## / \\ ##  /* * *\n"
...
VOID kuhl_m_sekurlsa_genericCredsOutput(PMANGO_GENERIC_PRIMARY_CREDENTIAL mesCreds, PLUID luid, ULONG flags)
{
...
VOID kuhl_m_sekurlsa_genericKeyOutput(PMANGO_CREDENTIAL_KEY key)
{
	switch(key->type)
	{
...
void kuhl_m_sekurlsa_krbtgt_keys(PVOID addr, LPCSTR prefix)
{
	...
	if(addr)
	{
		...
		if(NtBuildNumber < KULL_M_WIN_BUILD_10_1607)
		{
			if(ReadMemory((ULONG_PTR) addr, &tmpCred6, sizeof(MANGO_KRBTGT_CREDENTIALS_6) - sizeof(MANGO_KRBTGT_CREDENTIAL_6), NULL))
			{
		...
		{
			if(ReadMemory((ULONG_PTR) addr, &tmpCred64, sizeof(MANGO_KRBTGT_CREDENTIALS_64) - sizeof(MANGO_KRBTGT_CREDENTIAL_64), NULL))
			{
...
void kuhl_sekurlsa_dpapi_display_backupkey(ULONG_PTR pGuid, ULONG_PTR pPb, ULONG_PTR pCb, PCSTR text)
{
	...
	if(pGuid && pPb && pCb)
	{
		...
		if(ReadMemory(pCb, &cb, sizeof(DWORD), NULL) && ReadMemory(pPb, &tmpPtr, sizeof(PVOID), NULL))
		{
			if(cb && tmpPtr)
			{
				if(buffer = (PMANGO_BACKUP_KEY) LocalAlloc(LPTR, cb))
				{
