...
typedef struct _MANGO_HARD_KEY {
	ULONG cbSecret;
	BYTE data[ANYSIZE_ARRAY]; // etc...
} MANGO_HARD_KEY, *PMANGO_HARD_KEY;

typedef struct _MANGO_BCRYPT_KEY {
	ULONG size;
	ULONG tag;	// 'MSSK'
	ULONG type;
	ULONG unk0;
	ULONG unk1;
	ULONG unk2;
	MANGO_HARD_KEY hardkey;
} MANGO_BCRYPT_KEY, *PMANGO_BCRYPT_KEY;

typedef struct _MANGO_BCRYPT_KEY8 {
	ULONG size;
	ULONG tag;	// 'MSSK'
	ULONG type;
	ULONG unk0;
	...
	MANGO_HARD_KEY hardkey;
} MANGO_BCRYPT_KEY8, *PMANGO_BCRYPT_KEY8;

typedef struct _MANGO_BCRYPT_KEY81 {
	ULONG size;
	ULONG tag;	// 'MSSK'
	ULONG type;
	ULONG unk0;
	...
	ULONG unk9;
	MANGO_HARD_KEY hardkey;
} MANGO_BCRYPT_KEY81, *PMANGO_BCRYPT_KEY81;

typedef struct _MANGO_BCRYPT_HANDLE_KEY {
	ULONG size;
	ULONG tag;	// 'UUUR'
	PVOID hAlgorithm;
	PMANGO_BCRYPT_KEY key;
	PVOID unk0;
} MANGO_BCRYPT_HANDLE_KEY, *PMANGO_BCRYPT_HANDLE_KEY;

typedef struct _MANGO_BCRYPT_GEN_KEY {
	BCRYPT_ALG_HANDLE hProvider;
	BCRYPT_KEY_HANDLE hKey;
	PBYTE pKey;
	ULONG cbKey;
} MANGO_BCRYPT_GEN_KEY, *PMANGO_BCRYPT_GEN_KEY;
