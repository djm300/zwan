...
static SECPKG_FUNCTION_TABLE mangossp_SecPkgFunctionTable[] = {
	{
...
NTSTATUS NTAPI kssp_SpGetInfo(PSecPkgInfoW PackageInfo)
{
	...
	PackageInfo->Name       = L"MangoSSP";
	PackageInfo->Comment    = L"Mango Security Support Provider";
	return STATUS_SUCCESS;
}
...
#pragma warning(disable:4996)
	if(kssp_logfile = _wfopen(L"mangossp.log", L"a"))
#pragma warning(pop)
	{	
...
NTSTATUS NTAPI kssp_SpLsaModeInitialize(ULONG LsaVersion, PULONG PackageVersion, PSECPKG_FUNCTION_TABLE *ppTables, PULONG pcTables)
{
	...
	*ppTables = mangossp_SecPkgFunctionTable;
	*pcTables = ARRAYSIZE(mangossp_SecPkgFunctionTable);
	return STATUS_SUCCESS;
}