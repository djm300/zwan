...
NTSTATUS kkll_m_ssdt_list(PMANGO_BUFFER outBuffer)
{
	NTSTATUS status;
	USHORT idxFunction;
	ULONG_PTR funcAddr;
...
NTSTATUS kkll_m_ssdt_getKeServiceDescriptorTable()
{
	...
	if(KeServiceDescriptorTable)
		...
		status = kkll_m_memory_genericPointerSearch((PUCHAR *) &KeServiceDescriptorTable, ((PUCHAR) ZwUnloadKey) - (21 * PAGE_SIZE), ((PUCHAR) ZwUnloadKey) + (16 * PAGE_SIZE), PTRN_WALL_Ke, sizeof(PTRN_WALL_Ke), (MangoOsIndex < MangoOsIndex_8) ? OFFS_WNO8_Ke : (MangoOsIndex < MangoOsIndex_10_1607) ? OFFS_WIN8_Ke : OFFS_WIN10A_Ke);
	return status;
}
#endif